...
interface Task {
  ...
  morningFood: Food;
  eveningFood: Food;
  morningWetFood: number | string;
  morningDryFood: number | string;
  eveningWetFood: number | string;
  eveningDryFood: number | string;
}
...
const App: React.FC = () => {
  ...
  const [tasks, setTasks] = useState<Task[]>([
    {
      ...
      morningWetFood: placeholderMorningAmount,
      morningDryFood: placeholderMorningAmount,
      eveningWetFood: placeholderEveningAmount,
      eveningDryFood: placeholderEveningAmount,
    },
    {
      id: 2,
      title: "Dienstag",
  ...
  const [totalMorningDryFoodAmount, setTotalMorningDryFoodAmount] =
    useState<number>(0);
  const [totalEveningDryFoodAmount, setTotalEveningDryFoodAmount] =
    useState<number>(0);
  const [totalMorningWetFoodAmount, setTotalMorningWetFoodAmount] =
    useState<number>(0);
  const [totalEveningWetFoodAmount, setTotalEveningWetFoodAmount] =
    useState<number>(0);

  const updateTask = (task: Task) => {
    const updatedTasks = tasks.map((singleTask) =>
      singleTask.id === task.id ? task : singleTask
    );
  ...
  const handleSaveButtonClick = (id: number) => {
    ...
    if (taskToUpdate && !taskToUpdate.isDone) {
      const morningDryAmount =
        typeof taskToUpdate.morningDryFood === "string"
          ? 0
          : (taskToUpdate.morningDryFood as number);
      const morningWetAmount =
        typeof taskToUpdate.morningWetFood === "string"
          ? 0
          : (taskToUpdate.morningWetFood as number);
      const eveningDryAmount =
        typeof taskToUpdate.eveningDryFood === "string"
          ? 0
          : (taskToUpdate.eveningDryFood as number);
      const eveningWetAmount =
        typeof taskToUpdate.eveningWetFood === "string"
          ? 0
          : (taskToUpdate.eveningWetFood as number);

      setTotalMorningDryFoodAmount(
        (prevAmount) => prevAmount + morningDryAmount
      );
      setTotalMorningWetFoodAmount(
        (prevAmount) => prevAmount + morningWetAmount
      );
      setTotalEveningDryFoodAmount(
        (prevAmount) => prevAmount + eveningDryAmount
      );
      setTotalEveningWetFoodAmount(
        (prevAmount) => prevAmount + eveningWetAmount
      );
      ...
      const updatedTasks = tasks.map((task) =>
        task.id === id
          ? {
              ...task,
              isDone: false,
              morningFood: FoodType.Trockenfutter,
              eveningFood: FoodType.Trockenfutter,
              morningWetFood: 0,
              morningDryFood: 0,
              eveningWetFood: 0,
              eveningDryFood: 0,
  ...
  return (
    ...
    >
        ...
        {tasks.map((task) => (
          ...
          >
            ...
            <div className=" items-center flex flex-col">
              ...
              >
                <option value={FoodType.Trockenfutter}>Trockenfutter</option>
                <option value={FoodType.Nassfutter}>Nassfutter</option>
              </select>
              <input
                className="
                  text-center
                  ...
                  italic"
                type="number"
                value={task.morningFood.amount}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                  task.morningFood.amount = parseInt(e.target.value);
                }}
              />
            </div>
            <div className="items-center flex flex-col">
      ...
      >
        <div>
          Diese Woche Morgens Trockenfutter: {totalMorningDryFoodAmount} g
        </div>

        <div>Diese Woche Morgens Nassfutter: {totalMorningWetFoodAmount} g</div>
        <div>
          Diese Woche Abends Trockenfutter: {totalEveningDryFoodAmount} g
        </div>
        <div>Diese Woche Abends Nassfutter: {totalEveningWetFoodAmount} g</div>
      </div>
    </div>
  );
};
